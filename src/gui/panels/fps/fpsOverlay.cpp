...
void FpsOverlay::draw(const FrameTimer& timer)
{
    ...
    ImVec4 color;
    if      (fps >= 55.0f) color = ImVec4(0.35f, 0.95f, 0.45f, 1.0f);
    else if (fps >= 30.0f) color = ImVec4(0.95f, 0.85f, 0.20f, 1.0f);
    ...
    const bool statsVisible = (io.MousePos.x > io.DisplaySize.x - 160.0f) &&
                              (io.MousePos.y < 90.0f);
    if (!statsVisible)
    {
        ImGui::SetNextWindowPos(
            ImVec2(io.DisplaySize.x - margin, margin + 28.0f),
            ImGuiCond_Always,
            ImVec2(1.0f, 0.0f)
        );
